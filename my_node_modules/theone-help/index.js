 ...
 * face++ Pserson update
 */
exports.facePersonUpdate = function (perId, faceIds, cb) {
  facePP.person.add_face({
    'person_id':perId,
    'face_id':faceIds
  },function (err, res) {
    if(err){
 ...
 *  face++ 训练
 */
exports.faceTrainVerify =function  (perId, cb) {
  facePP.train.verify({
    'person_id':perId
 ...
 * face++ getSession
 */
exports.faceGetSession = function (sesId, cb) {
  // cb(null, sesId);
  facePP.info.get_singleSession(sesId,function (err, result){
    if(err){
      return cb(err);
    }
    cb(null, result);
...
/**
 * face++ 同步获取对比, person_id
 */
exports.faceRecongnitionVerify = function  (faceId, personId, cb ) {
  facePP.recognition.verify({
    'face_id':faceId,
    'person_id':personId
  },function  (err, result) {
    if(err){
...
/**
 * face++  删除数据， face++ 单纯的删除数据库，
 * !!是否需要去比对是否 faceplusplus.com 永存 存储
 */
